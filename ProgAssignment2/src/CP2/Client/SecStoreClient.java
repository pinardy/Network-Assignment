...
//        BufferedReader br = new BufferedReader(new FileReader("1001520.crt"));
        ...
        System.out.println("\nRequesting for certificate signed by CA from server");
        ...
        // Symmetric key crypto system for confidentiality protocol
        // Create AES session key
        KeyGenerator keyGenerator = KeyGenerator.getInstance("AES");
        SecretKey sessionKey = keyGenerator.generateKey();        
        
        // Prepare session key to send over to server
        byte[] keyBytes = sessionKey.getEncoded();
        byte[] encryptedKeyBytes = ecipher.doFinal(keyBytes);
        out.println(encryptedKeyBytes.length);
        System.out.println("\nEncrypted session key bytes sent to server");
        out.println(DatatypeConverter.printBase64Binary(encryptedKeyBytes));
        System.out.println("Encrypted session key sent to server");
        
        // Create and initialize AES cipher
